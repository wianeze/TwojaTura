...
function BarChart({ rows }: { rows: AnalyticsCountRow[] }) {
  ...
  return (
    <div className="space-y-3">
      {rows.length === 0 ? (
        <p className="text-sm text-[#806452]">Dane pojawią się po pierwszych wejściach.</p>
      ) : null}
      {rows.map((row) => (
        <div key={row.label} className="grid grid-cols-[7rem_1fr_auto] items-center gap-3 text-sm">
          ...
          <span className="h-3 overflow-hidden rounded-full bg-[#6e4a35]/12">
            <span
              ...
              style={{ width: `${Math.max(4, (row.eventsCount / max) * 100)}%` }}
            />
          </span>
    ...
    <div className="flex flex-col gap-2">
      {rows.map((row, index) => (
        <div
          ...
          style={{ width: `${Math.max(55, 100 - index * (45 / Math.max(1, rows.length - 1)))}%` }}
        >
          ...
          <span className="mt-1 block h-1 rounded-full bg-white/12">
            <span className="block h-full rounded-full bg-[#efb45e]" style={{ width: `${(row.eventsCount / max) * 100}%` }} />
          </span>
        </div>
      ))}
    </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="paper-wash rounded-[1.75rem] border border-[#8a5c3d]/24 p-5 shadow-[0_16px_38px_rgba(45,23,14,0.14)] sm:p-6">
      <h2 className="font-display mb-4 text-2xl font-semibold text-[#4b2f23]">{title}</h2>
      {children}
    </section>
  );
}
        ...
        <Section title="Zlecenia">
          <div className="flex items-center gap-5">
            <div
              ...
              style={{ background: `conic-gradient(#c67232 ${completionRate}%, rgba(105,70,48,.14) 0)` }}
            >
            ...
            <dl className="space-y-1 text-sm text-[#654838]">
              <div><dt className="inline">Wyświetlono: </dt><dd className="inline font-bold">{data.questUsage.presented}</dd></div>
              <div><dt className="inline">Kliknięto: </dt><dd className="inline font-bold">{data.questUsage.clicked}</dd></div>
              <div><dt className="inline">Wykonano: </dt><dd className="inline font-bold">{data.questUsage.completed}</dd></div>
              <div><dt className="inline">Po terminie: </dt><dd className="inline font-bold">{data.questUsage.expired}</dd></div>
            </dl>
          </div>
        </Section>
        <Section title="Lejek spotkania"><StepChart rows={data.funnel} /></Section>
        <Section title="Kontynuacje partii"><StepChart rows={data.continuations} /></Section>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Section title="Aktywność graczy">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[34rem] text-left text-sm">
              <thead className="text-xs tracking-wide text-[#936344] uppercase"><tr><th className="pb-3">Gracz</th><th>Logowania</th><th>Ostatnie logowanie</th><th>Ostatnia aktywność</th></tr></thead>
              <tbody className="divide-y divide-[#8d664d]/15">
                {data.userActivity.map((row) => <tr key={row.userId}><td className="py-3 font-semibold">{row.displayName}</td><td>{row.loginCount}</td><td>{formatAnalyticsDateTime(row.lastLoginAt)}</td><td>{formatDaysSince(row.lastActivityAt)}</td></tr>)}
              </tbody>
            </table>
          </div>
        </Section>
        <Section title="Ostatnie logowania">
          <div className="space-y-2">
            {data.recentLogins.length === 0 ? <p className="text-sm text-[#806452]">Brak zarejestrowanych logowań.</p> : null}
            {data.recentLogins.map((row, index) => <div key={`${row.createdAt}-${index}`} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[#8f6345]/16 bg-white/45 px-3 py-2 text-sm"><span><strong>{row.displayName}</strong><span className="ml-2 text-[#806452]">{row.deviceClass} · {row.browserFamily}</span></span><span className="text-[#806452]">{formatAnalyticsDateTime(row.createdAt)}</span></div>)}
          </div>
        </Section>
      </div>

      <div className="grid gap-6 xl:grid-cols-2">
        <Section title="Top komponenty i CTA"><BarChart rows={data.topComponents.map((row) => ({ ...row, label: `${row.label} · ${row.action}` }))} /></Section>
        <Section title="Najczęściej kontynuowane gry"><BarChart rows={data.continuedGames} /></Section>
      </div>

      {data.errors.length > 0 ? (
        <Section title="Ostatnie błędy akcji">
          <div className="space-y-2">{data.errors.map((row, index) => <div key={`${row.createdAt}-${index}`} className="grid gap-1 rounded-xl border border-[#a84935]/20 bg-[#fff4ec]/65 px-3 py-2 text-sm sm:grid-cols-[1fr_1fr_auto]"><strong>{row.errorAction || "Akcja serwerowa"}</strong><span>{row.displayName}</span><span className="text-[#806452]">{formatAnalyticsDateTime(row.createdAt)}</span></div>)}</div>
        </Section>
      ) : null}
    </div>
  );
